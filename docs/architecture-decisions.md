...
**Your decision:** Malware scanning is **required** in V1, not deferred. Files the scanner confirms as infected are **quarantined and never downloadable**. Files where scanning could not reach a verdict (`SCAN_FAILED` — e.g. the file was too large for the scanning provider to process, or a transient scanner/network error persisted through retries) are **not treated as infected**: a scan failure is a statement about the scanner, not about the file, so `SCAN_FAILED` documents remain downloadable, subject to the same organization/tenant/role authorization as any other document. The scanning provider/mechanism must be **replaceable** — i.e., the architecture must not hard-couple to a single vendor's SDK/API in a way that would require touching every call site to switch providers later.

**Implications for Stage 4:**
- `Document` gains a `scan_status` column: `PENDING` (just uploaded, not yet scanned) → `CLEAN` (scanner verified it safe) or `SCAN_FAILED` (scanner could not reach a verdict — treated the same as `CLEAN` for download purposes, never rewritten to `CLEAN` itself, so the real outcome stays visible for audit) → both downloadable; or `INFECTED` (scanner confirmed a positive match — quarantined, never downloadable).
- A document is **downloadable via signed URL** once `scan_status` is `CLEAN` **or** `SCAN_FAILED`; it remains blocked only while `PENDING` (still being scanned) or once confirmed `INFECTED`. This applies uniformly — no document type is exempt, including PODs and compliance documents. Download authorization for a `CLEAN`/`SCAN_FAILED` document is otherwise identical to any other document — the existing organization/tenant/role checks are unchanged and are not weakened by this rule.
- `INFECTED` and `SCAN_FAILED` documents are both moved to a shared `quarantine/` storage prefix (S3 has no distinct "reason" partition at the key level today) — this is a storage detail, not an authorization signal. Because `SCAN_FAILED` objects live in that same prefix, any future S3 bucket-policy hardening for defense-in-depth must **not** apply a blanket denial across the entire `quarantine/` prefix, as that would also block legitimate `SCAN_FAILED` downloads this decision requires to stay available. If prefix-level denial is ever wanted, it requires first splitting the prefix so `INFECTED` and `SCAN_FAILED` objects can be targeted independently — tracked as a separate future hardening item, not part of this decision.
...
**Exact decision locked:** Malware scanning is mandatory in V1 for every uploaded document. `Document.scan_status` (`PENDING`/`CLEAN`/`INFECTED`/`SCAN_FAILED`) gates download availability: `CLEAN` and `SCAN_FAILED` are downloadable (subject to unchanged normal authorization); `PENDING` and `INFECTED` are not. `SCAN_FAILED` is explicitly not equivalent to `INFECTED` — it means the scanner could not reach a verdict (commonly an oversized file), not that a threat was found. Scanning runs async via a background job. The scanner integration sits behind an internal interface so the provider can be swapped without a schema or call-site change. (Amended from the original V1 lock to match the shipped `isDocumentConsumable()` policy, commit `aaee429`.)
