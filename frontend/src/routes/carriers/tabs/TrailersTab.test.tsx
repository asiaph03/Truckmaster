...
function renderTab() {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <TrailersTab carrier={CARRIER} />
      <ToastViewport />
    </QueryClientProvider>,
  );
}

describe('TrailersTab — Add Trailer failure handling (Task #5)', () => {
  afterEach(() => {
  ...
  it('shows a toast error and keeps the modal open when the add request fails, and never shows success', async () => {
    ...
    server.use(
      http.post('/api/v1/carriers/carrier-1/trailers', () =>
        HttpResponse.json({ error: { code: 'INTERNAL_ERROR', message: 'boom' } }, { status: 500 }),
      ),
    );
    renderTab();

    fireEvent.click(screen.getByText('+ Add Trailer'));
    fireEvent.change(screen.getByLabelText(/^Unit Number/), { target: { value: 'TR-100' } });
    fireEvent.click(screen.getByRole('button', { name: 'Add Trailer' }));

    await waitFor(() => expect(screen.getByText('boom')).toBeInTheDocument());
    expect(screen.getByRole('dialog')).toBeInTheDocument();
    expect(screen.getByDisplayValue('TR-100')).toBeInTheDocument();
    expect(screen.queryByText('Trailer added.')).not.toBeInTheDocument();
  });
});
