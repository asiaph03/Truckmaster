...
import { FactoringTab } from './FactoringTab';
...
function renderTab() {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <FactoringTab carrier={CARRIER} />
      <ToastViewport />
    </QueryClientProvider>,
  );
}

describe('FactoringTab — Save failure handling (Task #5)', () => {
  afterEach(() => {
  ...
  it('shows a toast error and keeps the modal open when the save request fails, and never shows success', async () => {
    ...
    server.use(
      http.patch('/api/v1/carriers/carrier-1/factoring', () =>
    ...
    expect(screen.queryByText('Factoring info saved.')).not.toBeInTheDocument();
  });
});
