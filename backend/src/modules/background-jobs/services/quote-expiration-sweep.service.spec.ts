...
describe('QuoteExpirationSweepService — Monitoring Phase 4A-17 (sanitized error logging)', () => {
  ...
  it('the loadStaleQuotes (candidate-query) failure log contains only org correlation and errorType — no raw error content', async () => {
    const errorSpy = jest.spyOn(Logger.prototype, 'error').mockImplementation(() => undefined);
    const prisma = {
      organization: { findMany: jest.fn().mockResolvedValue([{ id: ORG_ID }]) },
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      `Quote expiration sweep: failed to load candidates for org ${ORG_ID}. errorType=Error`,
    );
  });

  it('SECURITY — the loadStaleQuotes failure log never contains a sensitive marker present in error.message/.stack/.meta', async () => {
    const logSpy = jest.spyOn(Logger.prototype, 'log').mockImplementation(() => undefined);
    const warnSpy = jest.spyOn(Logger.prototype, 'warn').mockImplementation(() => undefined);
    const errorSpy = jest.spyOn(Logger.prototype, 'error').mockImplementation(() => undefined);
    const prisma = {
  ...
  it('the per-quote failure log contains only org+entity correlation and errorType — no raw error content', async () => {
    const errorSpy = jest.spyOn(Logger.prototype, 'error').mockImplementation(() => undefined);
    const tx = {
      quote: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'quote-fail', status: 'OPEN', expirationDate: new Date('2020-01-01') }]),
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      `Quote expiration sweep: failed for org ${ORG_ID}, quote quote-fail. errorType=Error`,
    );
  });

  it('SECURITY — the per-quote failure log never contains a sensitive marker present in error.message/.stack/.meta', async () => {
    const logSpy = jest.spyOn(Logger.prototype, 'log').mockImplementation(() => undefined);
    const warnSpy = jest.spyOn(Logger.prototype, 'warn').mockImplementation(() => undefined);
    const errorSpy = jest.spyOn(Logger.prototype, 'error').mockImplementation(() => undefined);
    const tx = {
