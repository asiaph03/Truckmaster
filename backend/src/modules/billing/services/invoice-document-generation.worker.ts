...
export class InvoiceDocumentGenerationWorker implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    this.workerConnection = duplicateRedisWithErrorHandler(this.redis, 'invoice-pdf-worker');
    ...
    this.worker.on('completed', (job) => {
      const durationMs = job.processedOn ? Date.now() - job.processedOn : undefined;
      this.logger.log(
        `Invoice PDF job ${job.id} (org ${job.data.organizationId}) completed${durationMs !== undefined ? ` in ${durationMs}ms` : ''}.`,
      );
  ...
  async onModuleDestroy(): Promise<void> {
    this.heartbeat.unregister('invoice-pdf-worker');
