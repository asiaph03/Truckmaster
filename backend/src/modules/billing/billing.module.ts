  ...
  providers: [
    ...
    },
    {
      ...
      useFactory: (connection: Redis, queueRegistry: QueueRegistryService) => {
        const queue = new Queue(INVOICE_QUEUE_NAME, { connection });
        queueRegistry.register(INVOICE_QUEUE_NAME, queue);
        return queue;
      },
      inject: [INVOICE_QUEUE_CONNECTION, QueueRegistryService],
    },
  ],
})
